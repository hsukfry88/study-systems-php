...
class UploadFile {//类定义开始
     ...
     * @return string
     */
    public function upload($uploadName = '',$savePath ='') {
        if (!empty($uploadName)) {
            $this->config['uploadName'] = $uploadName;
        }
        //如果不指定保存文件名，则由系统默认
        if(empty($savePath))
    ...
    private function dealFiles($files) {
        if ($this->config['uploadName'] !== false) {
            $tmp[$this->config['uploadName']] = $files[$this->config['uploadName']];
            $files = $tmp;
        }
        $fileArray  = array();
        $n          = 0;
